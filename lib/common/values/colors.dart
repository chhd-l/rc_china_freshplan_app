import 'dart:ui';

class AppColors {
  /// 主背景 浅绿色
  static const Color primaryBackground = Color.fromRGBO(211, 228, 181, 1);

  /// 主文本颜色
  static const Color primaryText = Color.fromRGBO(0, 0, 0, 1);

  /// 按钮颜色
  static const Color tint = Color.fromRGBO(150, 204, 57, 1);

  /// 主文本颜色
  static const Color secondText = Color.fromRGBO(59, 58, 53, 1);

  /// 主文本颜色
  static const Color threeText = Color.fromRGBO(109, 109, 109, 1);

  /// tabCellSeparator 单元格底部分隔条 颜色
  static const Color tabCellSeparator = Color.fromARGB(255, 230, 230, 231);

  ///背景渐变1
  static const Color bgLinearGradient1 = Color.fromARGB(255, 195, 236, 123);

  ///背景渐变1
  static const Color bgLinearGradient2 = Color.fromARGB(0, 195, 236, 123);

  ///#999999
  static const Color text999 = Color.fromRGBO(153, 153, 153, 1);

  ///#666666
  static const Color text666 = Color.fromRGBO(102, 102, 102, 1);

  ///#333333
  static const Color text333 = Color.fromRGBO(51, 51, 51, 1);

  /// 绿色按钮禁用颜色
  static const Color baseGray=Color.fromRGBO(246, 246, 246, 1);
}
